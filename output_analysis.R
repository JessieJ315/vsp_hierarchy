...
plot(output$p[1:N],type='l',main='error probability for the queue-jumping noise 
     model',ylab='p')
abline(v=burn_in, col='red')
plot(output$theta[1:N],type='l',main="dispersion parameter for the Mallow's noise 
     model",ylab='theta')
abline(v=burn_in, col='red')
## latent matrices
...
showDAG(transitive.reduction(str2po(names(sort(PO_unlist,decreasing = TRUE)[2]),
                                    num_actors)))
### concensus order
...
## rho
ggplot(melt(data.frame(prior=rbeta(N-burn_in,1,RHO_HYPERPAMA),
                       posterior = output$rho[(burn_in+1):N])),
       aes(x=value,y=..density..,group=variable,color=variable,linetype=variable))+
  ...
  theme(legend.position="bottom",plot.title = element_text(hjust = 0.5),
        legend.title=element_blank(),text=element_text(family='Times New Roman'))
mean(output$rho[burn_in:N])
## tau
ggplot(melt(data.frame(prior=1/(1+exp(-rnorm(N-burn_in,sd=T_HYPERPAMA))),
                       posterior = output$tau[(burn_in+1):N])),
       aes(x=value,y=..density..,group=variable,color=variable,linetype=variable))+
  geom_density(alpha=0.4,adjust = 3)+
...
## p
ggplot(melt(data.frame(prior=1/(1+exp(-rnorm(N-burn_in,sd=R_HYPERPAMA))),
                       posterior = output$p[(burn_in+1):N])),
       aes(x=value,y=..density..,group=variable,color=variable,linetype=variable))+
  geom_density(alpha=0.4,adjust = 3)+
...
## theta
ggplot(melt(data.frame(prior=rgamma(N-burn_in,shape=THETA_HYPERPAMA,rate=1),
                       posterior = output$theta[(burn_in+1):N])),
       aes(x=value,y=..density..,group=variable,color=variable,linetype=variable))+
  geom_density(alpha=0.4,adjust = 3)+
